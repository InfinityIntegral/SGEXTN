...
template <typename FloatingPoint> void SGEXTN::SeerattraNum::WeibullDistribution<FloatingPoint>::setFailureBehaviour(FloatingPoint failureBehaviour){
    if(failureBehaviour < 0.0){SGEXTN_IMMEDIATE_CRASH("SGEXTN::SeerattraNum::WeibullDistribution::setFailureBehaviour crashed because requested failure behaviour indicator is negative");}
...
template <typename FloatingPoint> void SGEXTN::SeerattraNum::WeibullDistribution<FloatingPoint>::setCharacteristicLifespan(FloatingPoint characteristicLifespan){
    if(characteristicLifespan < 0.0){SGEXTN_IMMEDIATE_CRASH("SGEXTN::SeerattraNum::WeibullDistribution::setCharacteristicLifespan crashed because requested characteristic lifespan is negative");}
