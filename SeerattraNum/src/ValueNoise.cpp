...
SGEXTN::SeerattraNum::ValueNoise::ValueNoise(int dimension, SGEXTN::SeerattraNum::SmoothingFunction smoothingFunction) : dimension_(dimension), seed_(SGEXTN::SeerattraNum::TrueRandom::randomInt32()), smoothingFunction_(smoothingFunction){
...
float SGEXTN::SeerattraNum::ValueNoise::getHeight(const SGEXTN::Containers::Array<float>& point) const {
    if(point.length() != dimension_){SGEXTN_IMMEDIATE_CRASH("SGEXTN::SeerattraNum::ValueNoise::getHeight crashed because the number of dimensions in the point does not match that of the noise generator");}
    SGEXTN::Containers::Array<float> cornerHeights(powerOf2(dimension_), 0.0f);
    ...
    SGEXTN::Containers::Array<unsigned char> serialiseBuffer(4 * dimension_ + 4, static_cast<unsigned char>(0));
    bool isValid = false;
