...
template <typename ProbabilityType, typename Integer> SGEXTN::SeerattraNum::GeometricDistribution<ProbabilityType, Integer>::GeometricDistribution(bool useGlobal, ProbabilityType chanceOfTrue) : private_chanceOfTrue(chanceOfTrue), private_stlRandomEngine(nullptr), private_stlDistribution(SGEXTN::SeerattraNum::UnsafeCasts<std::geometric_distribution<Integer>>::eraseType(new std::geometric_distribution<Integer>(chanceOfTrue))){
    if(useGlobal == false){
