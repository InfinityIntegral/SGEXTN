...
SGEXTN::SeerattraNum::BernoulliDistribution::BernoulliDistribution(bool useGlobal, float chanceOfTrue) : chanceOfTrue_(chanceOfTrue), rngLocator_(useGlobal){
...
void SGEXTN::SeerattraNum::BernoulliDistribution::seed(const SGEXTN::Containers::Array<unsigned int>& seedArray){
    if(rngLocator_.isUsingGlobal() == true){SGEXTN_IMMEDIATE_CRASH("SGEXTN::SeerattraNum::BernoulliDistribution::seed crashed because cannot seed global rng");}
    (*rngLocator_).seed(seedArray);
}

bool SGEXTN::SeerattraNum::BernoulliDistribution::randomValue(){
    return ((*rngLocator_).randomFloat32() < chanceOfTrue_);
}
