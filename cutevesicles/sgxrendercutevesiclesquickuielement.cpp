...
QSGNode* SGXRenderCuteVesiclesQuickUIElement::updatePaintNode(QSGNode *thisNode, UpdatePaintNodeData *){
    ...
    (*mat).s = std::max(w, h);
    for(int i=0; i<10; i++){
        (*mat).velocity[i] += (SGXVector2(randomFloat((-1.0f) * (*mat).maxAccelerationPerSecond, (*mat).maxAccelerationPerSecond), randomFloat((-1.0f) * (*mat).maxAccelerationPerSecond, (*mat).maxAccelerationPerSecond)) / static_cast<float>((*mat).framesPerSecond));
        ...
        (*mat).center[i] += (*mat).velocity[i] / static_cast<float>((*mat).framesPerSecond);
        ...
        (*mat).radius[i] += randomFloat((-1.0f) * (*mat).maxRadiusChangePerSecond, (*mat).maxRadiusChangePerSecond) / (*mat).framesPerSecond;
        ...
        for(int j=0; j<8; j++){
            (*mat).radiusOffset[i][j] += randomFloat((-1.0f) * (*mat).maxRadiusOffsetChangePerSecond, (*mat).maxRadiusOffsetChangePerSecond);
