...
SGLArray<SGXColourRGBA> SGXThemeColours::generateSincerityUIThemeSet(SGXColourRGBA baseColour, bool darkMode){
    ...
    if(darkMode == false){
        arr.at(0) = SGXColourHSLA(baseColour).getColourWithContrast(0.6f);
        arr.at(1) = SGXColourHSLA(baseColour).getColourWithContrast(0.35f);
        ...
        arr.at(2) = SGXColourHSLA(baseColour).getColourWithContrast(2.0f);
        arr.at(3) = SGXColourHSLA(baseColour).getColourWithContrast(0.1f);
        arr.at(4) = SGXColourHSLA(baseColour).getColourWithContrast(0.35f);
        arr.at(5) = SGXColourHSLA(baseColour).getColourWithContrast(0.6f);
    }
    return arr;
}
