...
SGEXTN::ApplicationBase::TextBuffer::TextBuffer(char c){
    private_isHeapAlloc = false;
    (*private_stackAllocData) = static_cast<unsigned char>(c);
    private_lengthByte() = 1;
}

SGEXTN::ApplicationBase::TextBuffer::TextBuffer(const char* s){
    const int sLength = cStringLength(s);
    if(sLength < 24){
        private_isHeapAlloc = false;
        private_lengthByte() = sLength;
        for(int i=0; i<sLength; i++){
            (*(private_stackAllocData + i)) = static_cast<unsigned char>(*(s + i));
        }
    }
    else{
        private_isHeapAlloc = true;
        new(&private_heapAllocData) SGEXTN::Containers::Vector<unsigned char>();
        private_heapAllocData.reserve(sLength);
        private_heapAllocData.private_ringBuffer.private_length = sLength;
        for(int i=0; i<sLength; i++){
            (*private_heapAllocData.pointerToData(i)) = static_cast<unsigned char>(*(s + i));
        }
    }
}

SGEXTN::ApplicationBase::TextBuffer::TextBuffer(const TextBuffer& data, int start, int length){
    if(length < 24){
        private_isHeapAlloc = false;
        private_lengthByte() = length;
        if(data.private_isHeapAlloc == false){memoryCopy(data.private_stackAllocData + start, private_stackAllocData, length);}
        else{memoryCopy(data.private_heapAllocData.pointerToData(start), private_stackAllocData, length);}
    }
    else{
        private_isHeapAlloc = true;
        new(&private_heapAllocData) SGEXTN::Containers::Vector<unsigned char>();
        private_heapAllocData.reserve(length);
        private_heapAllocData.private_ringBuffer.private_length = length;
        memoryCopy(data.private_heapAllocData.pointerToData(start), private_heapAllocData.pointerToData(0), length);
    }
}
