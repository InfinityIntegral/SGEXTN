...
// 11 uppercase, 12 lowercase, 13 titlecase
...
void testToLowercaseOffsets(){
    parseFile();
    extractOffsets(12);
    qDebug() << "starting lowercase test";
    for(int i=0; i<=0x10ffff; i++){
        if(getLowercaseOffset(i) != offsetStorage.at(i)){
            qDebug() << i << offsetStorage.at(i) << getLowercaseOffset(i);
        }
    }
    qDebug() << "done lowercase test";
}
...
void testToTitlecaseOffsets(){
    parseFile();
    extractOffsets(13);
    qDebug() << "starting titlecase test";
    for(int i=0; i<=0x10ffff; i++){
        if(getTitlecaseOffset(i) != offsetStorage.at(i)){
            qDebug() << i << offsetStorage.at(i) << getTitlecaseOffset(i);
        }
    }
    qDebug() << "done titlecase test";
}

void changeDecimalToHex(const QString& s){
    QVector<QString> lines = s.split('\n');
    QString output = "";
    for(int i=0; i<lines.size(); i++){
        int start = lines.at(i).indexOf('(');
        int end = lines.at(i).indexOf(')');
        if(start == -1){output += (lines.at(i) + "\n"); continue;}
        QString conditions = lines.at(i).mid(start, end - start + 1);
        QString newConditions = "";
        QString buffer = "";
        for(int j=0; j<conditions.length(); j++){
            if(conditions.at(j).isDigit() == false){
                if(buffer != ""){
                    int num = buffer.toInt();
                    if(num < 3){newConditions += buffer;}
                    else{newConditions += (QString("0x") + QString::number(num, 16).toLower());}
                    buffer = "";
                }
                newConditions += conditions.at(j);
            }
            else{buffer += conditions.at(j);}
        }
        output += (lines.at(i).left(start) + newConditions + lines.at(i).right(lines.at(i).length() - end - 1) + "\n");
    }
