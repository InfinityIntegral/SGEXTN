...
void SGEXTN::Containers::UnitTests::testUnorderedSet(){
    ...
    x = s.erase(7);
    if(x == true){SGEXTN::Containers::Crash::crash("SGEXTN::Containers::UnorderedSet - erase nonexistent fail");}
    x = s.erase(2);
    if(s.length() != 5 || s.contains(2) == true || x == false){SGEXTN::Containers::Crash::crash("SGEXTN::Containers::UnorderedSet - erase fail");}
    if(s.contains(1) == false || s.contains(2) == true || s.contains(3) == false || s.contains(4) == false || s.contains(5) == false || s.contains(6) == false){SGEXTN::Containers::Crash::crash("SGEXTN::Containers::UnorderedSet - contains check fail");}
    SGEXTN::Containers::UnorderedSetIterator<int, SGEXTN::Containers::EqualTo<int>, SGEXTN::Containers::Hash<int>> itr = s.find(1);
    if((*itr) != 1){SGEXTN::Containers::Crash::crash("SGEXTN::Containers::UnorderedSet - find fail");}
    x = s.erase(itr);
    if(s.length() != 4 || s.contains(1) == true || x == false){SGEXTN::Containers::Crash::crash("SGEXTN::Containers::UnorderedSet - erase iterator fail");}
