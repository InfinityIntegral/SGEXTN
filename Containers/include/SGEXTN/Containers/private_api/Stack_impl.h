...
template <typename T> SGEXTN::Containers::Stack<T>::Stack() : ringBuffer_() {}

template <typename T> T& SGEXTN::Containers::Stack<T>::top(){
    if(ringBuffer_.length() == 0){SGEXTN_IMMEDIATE_CRASH("SGEXTN::Containers::Stack::top crashed as the stack is empty");}
...
template <typename T> void SGEXTN::Containers::Stack<T>::pop(){
    if(ringBuffer_.length() == 0){SGEXTN_IMMEDIATE_CRASH("SGEXTN::Containers::Stack::pop crashed as the stack is empty");}
    ringBuffer_.popBack();
}
