...
void SGRRenderingProgramme::finaliseShaderResource(){
    SGLVector<QRhiShaderResourceBinding> uniformBindings;
    {
        QRhiBuffer* uniformBuffer = (*rhi).newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::UniformBuffer, 32);
        (*uniformBuffer).create();
        uniformBindings.pushBack(QRhiShaderResourceBinding::uniformBuffer(0, QRhiShaderResourceBinding::VertexStage | QRhiShaderResourceBinding::FragmentStage, uniformBuffer));
    }
    for(int i=0; i<(*uniformBufferObjects).length(); i++){
        if((*uniformBufferObjects).at(i).second == 0){std::runtime_error("uniform buffer binding point 0 is reserved for the SG RI internal uniform buffer object which stores information about the custom renderer, pls attach your own uniform buffer objects to binding points starting from 1");}
        QRhiBuffer* uniformBuffer = (*rhi).newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::UniformBuffer, (*uniformBufferObjects).at(i).first);
        (*uniformBuffer).create();
        uniformBindings.pushBack(QRhiShaderResourceBinding::uniformBuffer((*uniformBufferObjects).at(i).second, QRhiShaderResourceBinding::VertexStage | QRhiShaderResourceBinding::FragmentStage, uniformBuffer));
    }
    shaderResources = (*rhi).newShaderResourceBindings();
    (*shaderResources).setBindings(uniformBindings.pointerToData(0), uniformBindings.pointerToData(uniformBindings.length()));
...
void SGRRenderingProgramme::finaliseRenderingProgramme(){
    if(shadersAreSet == false){
        std::runtime_error("you forgot to attach QSB shader binaries to the rendering programme, use SGRRenderingProgramme::setShaderQSBFiles to attach them");
    }
    if(vertexFormattingIsSet == false){
        std::runtime_error("you forgot to specify the type of data taken as input by your vertex shader and the location of that data, use SGRRenderingProgramme::finaliseVertices to specify these, you must finalise vertex properties after adding them");
    }
...
void SGRRenderingProgramme::updateShaderUniforms(int shaderDeclaredBinding, int startLocation, int dataSize, void *pointerToData){
    ...
    if(buffer == nullptr){std::runtime_error("you attempted to upload uniforms to a nonexistent binding point");}
