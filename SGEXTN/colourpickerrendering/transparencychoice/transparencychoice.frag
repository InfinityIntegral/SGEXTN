...
layout(std140, binding = 0) uniform uniformBuffer{
	...
	uniform float selectedLightness;
	uniform float selectedTransparency;
};
...
vec4 getRGB(float xh, float xs, float xl){
	float r = 0.0;
	float g = 0.0;
	float b = 0.0;
	if(xs == 0.0){
...
void main(){
	...
	float offsetX = (newCoordX - selectedTransparency) * sizeW / sizeH * 2.0;
	float offsetY = (1.0 - coordY);
	if(abs(offsetX) + abs(offsetY) < 0.5){
		oc = getRGB(selectedHue, selectedSaturation, selectedLightness);
		oc = vec4(selectedTransparency * oc.r + (1.0 - selectedTransparency) * bc, selectedTransparency * oc.g + (1.0 - selectedTransparency) * bc, selectedTransparency * oc.b + (1.0 - selectedTransparency) * bc, 1.0);
	}
	else if(coordY > 0.8 || newCoordX < 0.0 || newCoordX > 1.0){oc = vec4(1.0, 1.0, 1.0, 0.0);}
	else{
		oc = getRGB(selectedHue, selectedSaturation, selectedLightness);
		oc = vec4(newCoordX * oc.r + (1.0 - newCoordX) * bc, newCoordX * oc.g + (1.0 - newCoordX) * bc, newCoordX * oc.b + (1.0 - newCoordX) * bc, 1.0);
	}
}
