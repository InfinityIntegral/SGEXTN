...
void main(){
	...
	float offsetX = (newCoordX - selectedLightness) * sizeW / sizeH * 2.0;
	float offsetY = (1.0 - coordY);
	if(abs(offsetX) + abs(offsetY) < 0.5){oc = getRGB(selectedHue, selectedSaturation, selectedLightness);}
	else if(coordY > 0.8 || newCoordX < 0.0 || newCoordX > 1.0){oc = vec4(1.0, 1.0, 1.0, 0.0);}
	else{oc = getRGB(selectedHue, selectedSaturation, newCoordX);}
}
