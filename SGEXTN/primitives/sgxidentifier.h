...
public:
    ...
    SGXIdentifier(const QString& s, bool ifValid, int& errCode); // generates identifier from string representation with ifValid determining if it cannot be the same as an already registered identifier, if ifValid is true and the identifier already exists, it is created anyways but errCode is set to 1 instead of 0, errCode is set to 2 instead of 0 if the data produces a null identifier, returns null identifier if input string is invalid
