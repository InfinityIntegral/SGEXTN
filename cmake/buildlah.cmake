macro(BuildLah_use_default_settings)
    message(STATUS "you are using SGEXTN version 6.4.0, BuildLah CMake integration")
    ...
    if(APPLE)
        set(CMAKE_BUILD_RPATH "@executable_path/../Frameworks;@loader_path/../Frameworks;@executable_path/Frameworks;@loader_path/Frameworks")
    ...
    if(NOT DEFINED ${CMAKE_BUILD_TYPE})
        message(WARNING "build type is not defined, use -DCMAKE_BUILD_TYPE=Release in the configure command to select a release build")
...
macro(BuildLah_find_dependencies)
    ...
    if(${PROJECT_NAME} STREQUAL "SGEXTN")
        ...
        foreach(comp IN ITEMS Containers Math Core FileSystem SGWidget RendererInterface EntryPoint)
    ...
    file(APPEND ${BUILDLAH_VERSION_PATH} "SGEXTN version: 6.4.0\n")
...
macro(BuildLah_add_component arg_target_name arg_target_type arg_files_path)
    ...
    file(WRITE "${CMAKE_CURRENT_BINARY_DIR}/BuildLah_internal_DLL.h" "#pragma once\n\n#define BUILDLAH_INTERNAL_WINDOWS_DLL_EXPORT __declspec(dllexport)\n#define BUILDLAH_INTERNAL_WINDOWS_DLL_IMPORT __declspec(dllimport)\n#define BUILDLAH_INTERNAL_UNIX_DLL __attribute__((visibility(\"default\")))\n")
    if(MSVC)
        target_compile_options(${arg_target_name} PUBLIC /FI"${CMAKE_CURRENT_BINARY_DIR}/BuildLah_internal_DLL.h")
        target_compile_options(${arg_target_name} PRIVATE /utf-8)
    else()
        target_compile_options(${arg_target_name} PUBLIC -include "${CMAKE_CURRENT_BINARY_DIR}/BuildLah_internal_DLL.h")
        target_compile_options(${arg_target_name} PRIVATE -finput-charset=UTF-8 -fexec-charset=UTF-8)
    endif()

    if(WIN32)
        target_compile_definitions(${arg_target_name} PRIVATE BUILDLAH_DLL_${PROJECT_NAME}_${arg_target_name}=BUILDLAH_INTERNAL_WINDOWS_DLL_EXPORT)
        target_compile_definitions(${arg_target_name} INTERFACE BUILDLAH_DLL_${PROJECT_NAME}_${arg_target_name}=BUILDLAH_INTERNAL_WINDOWS_DLL_IMPORT)
    else()
        target_compile_definitions(${arg_target_name} PUBLIC BUILDLAH_DLL_${PROJECT_NAME}_${arg_target_name}=BUILDLAH_INTERNAL_UNIX_DLL)
    endif()
    ...
    qt6_add_resources(${arg_target_name} ${arg_target_name}_assets PREFIX "${PROJECT_NAME}_${arg_target_name}" BASE "${arg_files_path}/assets" FILES ${ASSETS_FILES})
    ...
    foreach(asset_file ${ASSETS_FILES_NAMEONLY})
        message(STATUS "added ${arg_files_path}/assets/${asset_file} to Qt Resource System of ${arg_target_name} as :/${PROJECT_NAME}_${arg_target_name}/${asset_file}")
    endforeach()
    ...
    qt6_add_shaders(${arg_target_name} ${arg_target_name}_shaders PREFIX "${PROJECT_NAME}_${arg_target_name}" BASE "${arg_files_path}/shaders" FILES ${SHADERS_FILES})
    ...
    foreach(shader_file ${SHADERS_FILES_NAMEONLY})
        message(STATUS "the shader at ${arg_files_path}/shaders/${shader_file} has been compiled using QSB and added to Qt Resource System of ${arg_target_name} as :/${PROJECT_NAME}_${arg_target_name}/${shader_file}.qsb")
    endforeach()
